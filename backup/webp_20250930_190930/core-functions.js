...
console.log('🔧 BALANCE FIX VERSION - 2025-09-27 - USDT余额修复版本已加载');
            ...
            if (typeof window.Web3 !== 'undefined') {
                try {
                    ...
                    try {
                        if (typeof window.getBestRPC === 'function') {
                            const bestRPC = await window.getBestRPC();
                            window.web3 = new window.Web3(bestRPC);
                            ...
                            console.log('✅ Web3 初始化完成 (使用最佳RPC)');
                        } else if (typeof window.createWeb3InstanceSync === 'function') {
                            window.web3 = window.createWeb3InstanceSync();
                            ...
                            console.log('✅ Web3 初始化完成 (使用默认RPC)');
                        } else {
                            throw new Error('无法创建Web3实例');
                        }
                    } catch (fallbackError) {
                        console.error('❌ Web3备用初始化也失败:', fallbackError);
                        showMessage('Web3 initialization failed, please refresh and try again', 'error');
                        return false;
                    }
                }
            } else {
...
async function checkNetworkHealth() {
    try {
        ...
        // 检查响应时间 - 放宽限制，避免误判
        if (responseTime > 10000) { // 从 5000ms 增加到 10000ms
            console.warn(`⚠️ 网络响应慢: ${responseTime}ms`);
            return {
                healthy: false,
                reason: `Slow response: ${responseTime}ms`,
                responseTime
            };
        }
